...
# How many recent messages to include as LLM context (5 user + 5 assistant turns)
CONTEXT_WINDOW = 10
...
def get_llm_response(api_key: str, persona_name: str) -> str:
    """Build message list from session history and invoke the LLM via LCEL."""
    llm = get_or_create_llm(api_key)
    chain = llm | StrOutputParser()

    lc_messages = [SystemMessage(content=PERSONAS[persona_name])]
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                response = get_llm_response(api_key, st.session_state.current_persona)
